"""SYSTEM OSCILLATOR v3.2 wave programs: v3.1 with the stock FINE IN scale. Offline; no device I/O.

Jacks (unchanged from v3): in1 CV IN, in2 FINE IN (1.0 = 0.1 octave = 1.2 semitones, the stock SAW/SQR scale), in3 COLOR IN (+ P3/100, now clamped 0..1),
...
Adapter per wave (every record is a native Roland record shape):
  pitch  1e05 0fe0 2fe2 | IN2 IN1            r0 = FINE, r2 = CV
         0805 0422      | 0002 <1/100>       r0 = FINE/100 + CV          (SYSTEM-1m base rec 31 shape)
...
assert abs(c32(*FINE_K) - 1 / 100) < 1e-9 and c32(*TEN) == 10.0
...
def build_program(slot, cell):
    ...
    pitch = [0x1e05, 0x0fe0, 0x2fe2, IN[1], IN[0],
             0x0805, 0x0422, 0x0002, FINE_K[0], FINE_K[1],
